...
function getTableRows($db, $where = "", $sort = "")
{
...
function getItem($db, $sql)
{
    $select = $db->select($sql);

    return isset($select[0]) ? $select[0] : [];
}

// Delete item from database
function deleteItem($db, $sql)
{
    return $db->delete($sql);
}

// Add new item to database
function addItem($db, $title, $year, $format, $cast)
{
